/**
 * Font configuration helper for Settings app
...
/**
 * Preload special fonts for languages that require them (e.g., Chinese)
 * @param {string} languageCode - Language code
 * @param {Object} translationObj - Translation object with _meta.font
 * @returns {Promise<void>}
 */
export async function preloadLanguageFonts(languageCode, translationObj = null) {
    const config = getFontConfig(translationObj);
    
    // Chinese languages need DFPHeiW5-GB font
    if (languageCode === 'zh-Hans-CN' || languageCode === 'zh-Hant') {
        try {
            const font = new FontFace('DFPHeiW5-GB', 'url("/content/common/fonts/DFPHeiW5-GB.ttf")');
            await font.load();
            document.fonts.add(font);
            console.log('[Fonts] Preloaded DFPHeiW5-GB font for Chinese');
        } catch (error) {
            console.warn('[Fonts] Failed to preload DFPHeiW5-GB font:', error);
        }
    }
}
