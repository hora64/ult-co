...
export async function createWaterMaterial(options = {}) {
  // Use absolute paths for oceanDemo textures
  const texture1Path = options.textureOne || '/content/apps/oceanDemo/banner/textures/water1.png';
  const texture2Path = options.textureTwo || '/content/apps/oceanDemo/banner/textures/water2.png';
  
  console.log('[WaterMaterial] Loading textures with caching:', { texture1Path, texture2Path });
  
  // Load textures with IndexedDB caching
  try {
    ...
    console.log('[WaterMaterial] Textures loaded successfully');
    ...
    console.error('[WaterMaterial] Failed to load textures:', error);
    throw error;
  }
}