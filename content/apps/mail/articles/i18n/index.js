// Lazy loading article translation modules for Mail app

/**
 * Dynamically load article translations for a language
 * @param {string} languageCode - Language code (e.g., 'en-US')
 * @returns {Promise<Array>} Array of articles for the language
 */
export async function loadArticles(languageCode) {
    try {
        switch (languageCode) {
            case 'en-US':
            ...
            default:
                console.warn(`Articles for language ${languageCode} not found, falling back to en-US`);
                return (await import('./en-US.js')).enUS;
        }
    } catch (error) {
        console.error(`Failed to load articles for ${languageCode}:`, error);
        return (await import('./en-US.js')).enUS;
    }
}

/**
 * Get list of supported languages for articles
 ...
 * Note: This loads all articles at once - use loadArticles() for lazy loading
 */
