...
/**
 * AppIconRenderer - Shared rendering logic for app icons
 ...
 */
export class AppIconRenderer {
    /**
     * Renders an app icon on a canvas
    ...
    static async renderIcon(canvas, options) {
        const {
            ...
            animated = false, // NEW: Support for animated icons
            animationFrame = 0 // NEW: Current animation frame
        } = options;
        ...
        if (baseIcon) {
            // BaseIcon apps: Use background/base as the static layer
            ...
            // Regular apps: Use background + overlay pattern
            if (unopened) {
        ...
        try {
            ...
            console.error('AppIconRenderer: Failed to draw base icon', error);
        ...
        if (overlaySrc) {
            try {
                ...
                console.warn('AppIconRenderer: Could not load overlay icon, showing base only', error);
            }
        }
    }
    ...
    static async renderSimpleIcon(canvas, iconPath, size, animated = false) {
        ...
        try {
            ...
            console.error('AppIconRenderer: Failed to render simple icon', error);
    ...
    /**
     * Preloads icons for an array of apps
     * @param {Array<Object>} apps - Array of app data
     * @returns {Promise<void>}
     */
    static async preloadAppIcons(apps) {
        const iconPaths = new Set();

        apps.forEach(app => {
            if (app.icon) iconPaths.add(app.icon);
            if (app.actualIcon) iconPaths.add(app.actualIcon);
            if (app.baseIcon) iconPaths.add(app.baseIcon);
            if (app.unopenedIcon) iconPaths.add(app.unopenedIcon);
            if (app.backgroundIcon) iconPaths.add(app.backgroundIcon);
            if (app.wrapIcon) iconPaths.add(app.wrapIcon);
        });
