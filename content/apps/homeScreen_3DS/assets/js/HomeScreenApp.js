    ...
    async main() {
        try {
            ...
            if (userLanguage === 'zh-Hans-CN' || userLanguage === 'zh-Hant') {
                try {
                    console.log('[HomeScreenApp] Preloading Chinese font DFPHeiW5-GB...');
                    ...
                    console.log('[HomeScreenApp] Chinese font DFPHeiW5-GB loaded successfully');
                } catch (error) {
                    console.warn('[HomeScreenApp] Failed to preload Chinese font, will use fallbacks:', error);
                }
            }

            // Note: Do NOT apply font to document.body here, as it would override
            // fonts in Mail and Settings apps. Each app should manage its own fonts.
            // HomeScreen elements (buttons, top screen) use their own font styling.
